...
function toggleDarkMode() {
  window.userSettings.theme = window.userSettings.theme === "light" ? "dark" : "light"
  applyTheme(window.userSettings.theme)
  updateDarkModeToggle(window.userSettings.theme)
  persistSettings()
}
...
function updateDarkModeToggle(theme) {
  ...
  if (icon) icon.className = theme === "dark" ? "fas fa-sun mr-2" : "fas fa-moon mr-2"
  if (text) text.textContent = theme === "dark" ? "Light Mode" : "Dark Mode"
}

function persistSettings() {
  localStorage.setItem("taskmaster_settings", JSON.stringify(window.userSettings))
...
function getSetting(key, defaultValue = null) {
  return window.userSettings[key] !== undefined ? window.userSettings[key] : defaultValue
}
...
function updateSettingsFromExternal(newSettings) {
  ...
  Object.keys(newSettings || {}).forEach((key) => {
    if (window.userSettings?.[key] !== newSettings[key]) {
      window.userSettings[key] = newSettings[key]
      updated = true
    }
  })
  if (updated) {
...
function getUserId() {
  try {
    const raw = localStorage.getItem("taskmaster_user")
    if (!raw) return null
...
async function syncTaskUpdate(task) {
  ...
  try {
    ...
    if (task.completed && window.userSettings.taskSounds) {
      playCompletionSound()
    }
  } catch (error) {
...
function playCompletionSound() {
  try {
    const enabled = window.userSettings?.["task-sounds"] ?? window.userSettings?.taskSounds
    if (!enabled) return
...
function initializeDateInputs() {
  ...
  const editDue = document.getElementById("edit-task-due-date")
  if (due) due.setAttribute("min", today)
  if (editDue) editDue.setAttribute("min", today)
}
...
function setupEventListeners() {
  ...
  document
    ...
    .forEach((btn) => btn.addEventListener("click", handlePriorityFilterChange))
...
function handleVisibilityChange() {
  if (!document.hidden) {
    ...
    if (!lastSettingsUpdate || currentTime - Number.parseInt(lastSettingsUpdate) > 5000) {
      loadSettingsFromDB()
        .then(() => {
          applyUserSettings()
        })
...
async function loadSettingsFromDB() {
  try {
    ...
    if (res.ok) {
      ...
      window.userSettings.theme = settings.theme ?? window.userSettings.theme
      window.userSettings["task-sounds"] =
        settings["task-sounds"] ?? settings.taskSounds ?? window.userSettings["task-sounds"]
      window.userSettings["due-date-reminders"] =
        settings["due-date-reminders"] ?? settings.dueDateReminders ?? window.userSettings["due-date-reminders"]
      window.userSettings["email-notifications"] =
        settings["email-notifications"] ?? settings.emailNotifications ?? window.userSettings["email-notifications"]
      window.userSettings["task-reminders"] =
        settings["task-reminders"] ?? settings.taskReminders ?? window.userSettings["task-reminders"]
      window.userSettings["default-priority"] =
        settings["default-priority"] ?? settings.defaultPriority ?? window.userSettings["default-priority"]
      window.userSettings["tasks-per-page"] =
        settings["tasks-per-page"] ?? settings.tasksPerPage ?? window.userSettings["tasks-per-page"]
      localStorage.setItem("taskmaster_settings", JSON.stringify(window.userSettings))
      localStorage.setItem("taskmaster_settings_timestamp", Date.now().toString())
      console.log("Settings loaded from backend:", window.userSettings)
    } else {
      const local = localStorage.getItem("taskmaster_settings")
      if (local) window.userSettings = JSON.parse(local)
    }
  } catch (err) {
...
function applyUserSettings() {
  ...
  applySettingsToUI(window.userSettings)
}
...
window.SettingsManager = {
  ...
  getCurrentSettings: () => ({ ...window.userSettings }),
}

// Log initialization
console.log("Dashboard initialized with settings:", window.userSettings)
